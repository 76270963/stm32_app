...
void usartRead(void) //串口数据解析
{
	uint16_t Length;
	uint16_t crc;
	Length = (buffer_RX1[3] << 8) + buffer_RX1[4];
	crc = GetCrc_16(buffer_RX1, Length + 5);
	if((buffer_RX1[Length + 5] == (crc & 0xFF)) && (buffer_RX1[Length + 6] == (crc >> 8)))
	{
		//if((buffer_RX1[2] <= 4)||(buffer_RX1[2] >= 1))
			//BootLoadAPP(buffer_RX1);
	}
}
